...
describe("StakingContract", function () {
  ...
  beforeEach(async function () {
    ...
    await navToken.mint(deployerAddress, initialRewardTokens.add(initialStakeTokens.mul(2)));
    ...
    // Transfer stake tokens to stakers
  ...
  describe("NavFinance Token Specifics", function () {
    it("should have the correct name and symbol", async function () {
      expect(await navToken.name()).to.equal("NavFinance");
      expect(await navToken.symbol()).to.equal("NAV");
    });

    it("should allow minting by the owner", async function () {
      const initialBalance = await navToken.balanceOf(deployerAddress);
      await navToken.mint(deployerAddress, parseEther("1000"));
      const finalBalance = await navToken.balanceOf(deployerAddress);
      expect(finalBalance.sub(initialBalance)).to.equal(parseEther("1000"));
    });

    it("should not allow minting by non-owners", async function () {
      await expect(navToken.connect(stakerOne).mint(stakerOneAddress, parseEther("1000")))
